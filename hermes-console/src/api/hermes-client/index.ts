...
export function createRetransmissionTask(task: OfflineRetransmissionCreateTask) {
  return axios.post(`/offline-retransmission/tasks`, task, {
    headers: { 'Content-Type': 'application/json' },
  });
}

export function getActiveRetransmissions(topicName: String): ResponsePromise<Array<OfflineRetransmissionActiveTask>> {
