...
describe('SubscriptionsList', () => {
  ...
  it.each(props.subscriptions)(
    ...
    async (subscription) => {
      ...
      expect(getByText(subscription.name).closest('a')).toHaveAttribute(
        'href',
        `/ui/groups/pl.allegro/topics/pl.allegro.DummyTopic/subscriptions/${subscription.name}`,
      );
    },
  );
  ...
  it.each(['foobar', 'FOOBAR', 'FooBar'])(
    ...
    async (filter: string) => {
      // given
      const { getByLabelText, queryByText } = render(SubscriptionsList, {
        props,
      });
  ...
  it('should not render copy clients button when there are no subscriptions', async () => {
    ...
    // when
    const { queryByText } = render(SubscriptionsList, {
