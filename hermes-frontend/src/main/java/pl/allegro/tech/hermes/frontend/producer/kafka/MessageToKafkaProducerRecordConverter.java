...
public class MessageToKafkaProducerRecordConverter {
    ...
    public ProducerRecord<byte[], byte[]> convertToProducerRecord(
        ...
        byte[] partitionKey =
                ofNullable(message.getPartitionKey()).map(String::getBytes).orElse(null);
    ...
    private Iterable<Header> createRecordHeaders(Message message) {
        ...
        message.<Schema>getCompiledSchema()
                .ifPresent(
                        compiledSchemaVersion -> {
                            headers.add(
                                    kafkaHeaderFactory.schemaVersion(
                                            compiledSchemaVersion.getVersion().value()));
                            if (schemaIdHeaderEnabled) {
                                headers.add(
                                        kafkaHeaderFactory.schemaId(
                                                compiledSchemaVersion.getId().value()));
                            }
                        });
