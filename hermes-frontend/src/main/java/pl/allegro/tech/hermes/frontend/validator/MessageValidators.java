...
public class MessageValidators implements TopicCallback {
    ...
    public void check(TopicName topicName, byte[] message) {
        Optional.ofNullable(topicsWithValidators.get(topicName)).ifPresent(validator -> {
            Timer.Context globalValidationTimerContext = hermesMetrics.timer(Timers.PRODUCER_VALIDATION_LATENCY).time();
            ...
            try {
                ...
                globalValidationTimerContext.stop();
                topicValidationTimerContext.stop();
            }
        });
    }
