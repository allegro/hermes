...
public class TopicSchemaLoadingStartupHook {
    ...
    private static final Logger logger =
            LoggerFactory.getLogger(TopicSchemaLoadingStartupHook.class);
    ...
    private List<SchemaLoadingResult> loadSchemasForTopics(List<Topic> topics) {
        try (TopicSchemaLoader loader =
            ...
            return allComplete(topics.stream().map(loader::loadTopicSchema).collect(toList()))
                    .join();
        } catch (Exception e) {
    ...
    private void logResultInfo(List<SchemaLoadingResult> allResults, long elapsed) {
        ...
        Optional<List<SchemaLoadingResult>> missing =
                Optional.ofNullable(groupedResults.get(MISSING));
        Optional<List<SchemaLoadingResult>> failures =
                ...
                missing.map(
                                results ->
                                        String.format(
                                                ...
                                                topicsOfResults(results)))
                        .orElse(""),
                failures.map(
                                results ->
                                        String.format(
                                                "Failed for: [%s]. ", topicsOfResults(results)))
                        .orElse(""));
    }
