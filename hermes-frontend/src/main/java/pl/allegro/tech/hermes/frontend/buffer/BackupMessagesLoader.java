...
public class BackupMessagesLoader {
    ...
    public void loadFromTemporaryBackupV2File(File file) {
        try (FileInputStream fileInputStream = new FileInputStream(file);
            ...
            logger.info(
                    "Loaded {} messages from temporary v2 backup file: {}", messages.size(), file);
            loadMessages(messages);

        } catch (IOException | ClassNotFoundException e) {
            logger.error(
                    "Error reading temporary backup v2 files from path {}.",
    ...
    private void resendMessages(List<Pair<Message, CachedTopic>> messageAndTopicList, int retry) {
        logger.info(
                "Resending {} messages from backup storage retry {}.",
