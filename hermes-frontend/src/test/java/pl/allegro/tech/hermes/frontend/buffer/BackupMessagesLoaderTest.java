...
public class BackupMessagesLoaderTest {
    ...
    private final SchemaExistenceEnsurer schemaExistenceEnsurer =
            mock(SchemaExistenceEnsurer.class);
    ...
    public void setUp() {
        ...
        when(cachedTopic.startBrokerLatencyTimer())
                .thenReturn(HermesTimerContext.from(micrometerTimer));
    ...
    public void shouldNotSendOldMessages() {
        ...
        LocalMessageStorageProperties localMessageStorageProperties =
                new LocalMessageStorageProperties();
        ...
        verify(producer, times(1))
                .send(any(JsonMessage.class), eq(cachedTopic), any(PublishingCallback.class));
    }
    ...
    public void shouldSendAndResendMessages() {
        ...
        doAnswer(
                        invocation -> {
                            ...
                            ((PublishingCallback) args[2])
                                    .onUnpublished(
                                            (Message) args[0],
                                            ((CachedTopic) args[1]).getTopic(),
                                            new Exception("test"));
                            return "";
                        })
                .when(producer)
        ...
        // then
        verify(producer, times(noOfSentCalls))
                .send(any(JsonMessage.class), eq(cachedTopic), any(PublishingCallback.class));
        verify(listeners, times(noOfSentCalls))
                .onError(any(JsonMessage.class), eq(topic), any(Exception.class));
    }
    ...
    public void shouldSendOnlyWhenBrokerTopicIsAvailable() {
        ...
        when(producer.isTopicAvailable(cachedTopic))
                .thenReturn(false)
                .thenReturn(false)
                .thenReturn(true);
    ...
    public void shouldSendMessageWithAllArgumentsFromBackupMessage() {
        ...
        verify(producer, times(1))
                .send(messageArgument.capture(), eq(cachedTopic), any(PublishingCallback.class));
        Message sendMessage = messageArgument.getValue();
        assertThat(sendMessage.getPartitionKey())
                .isEqualTo(backupMessages.get(0).getPartitionKey());
        ...
        assertThat(sendMessage.getHTTPHeaders().get("propagated-http-header"))
