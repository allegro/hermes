...
class DynamicThroughputLimiterTest extends Specification {
    ...
    def globalMeter = [getOneMinuteRate: {globalRate}] as HermesRateMeter
    ...
    def "should block single abuser among multiple users"() {
        ...
        def abuserMeter = [getOneMinuteRate: {abuserRate}] as HermesRateMeter
    ...
    private HermesRateMeter meter(double rate) {
        Mock(HermesRateMeter) { getOneMinuteRate() >> rate}
    }

    private TopicName registerTopic(String group, String name, HermesRateMeter rate) {
