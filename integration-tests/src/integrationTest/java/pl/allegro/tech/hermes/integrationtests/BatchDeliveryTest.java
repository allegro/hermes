...
public class BatchDeliveryTest {
    ...
    private static final MessageFilterSpecification MESSAGE_NAME_FILTER =
            new MessageFilterSpecification(
                    of("type", "jsonpath", "path", ".name", "matcher", "^Bob.*"));
        ...
        Topic topic =
                hermes.initHelper()
                        .createTopicWithSchema(
                                topicWithSchema(
                                        topicWithRandomName().build(), user.getSchemaAsString()));
    ...
    public void shouldPassSubscriptionHeaders() {
        ...
        subscriber.waitUntilRequestReceived(
                request -> {
                    Assertions.assertThat(request.getHeader("MY-HEADER"))
                            .isEqualTo("myHeaderValue");
                        ...
                        (service, endpoint) -> {
                            ...
                            service.addStubMapping(
                                    post(urlEqualTo(endpoint))
                                            ...
                                            .willReturn(
                                                    aResponse().withStatus(200).withFixedDelay(0))
                                            .build());
                        });
    ...
    private void publishAll(String topicQualifiedName, TestMessage... messages) {
        stream(messages)
                .forEach(
                        message ->
                                hermes.api()
                                        .publishUntilSuccess(topicQualifiedName, message.body()));
    }
                ...
                message -> {
                    ...
                    for (int i = 0; i < expectedContents.length; i++) {
                        ...
                        Assertions.assertThat(
                                        (String) ((Map) batch.get(i).get("metadata")).get("id"))
                                .isNotEmpty();
                    }
                });
    }
