...
public class BatchDeliveryTest {
    ...
    public void shouldDeliverMessagesInBatch() {
        ...
        Subscription subscription = subscription(topic.getQualifiedName(),
    ...
    public void shouldDeliverAvroMessagesAsJsonBatch() {
        ...
        Topic topic = hermes.initHelper().createTopicWithSchema(
    ...
    public void shouldPassSubscriptionHeaders() {
        ...
        subscriber.waitUntilRequestReceived(request -> {
            ...
            Assertions.assertThat(request.getHeader("MY-OTHER-HEADER")).isEqualTo("myOtherHeaderValue");
        });
    }
    ...
    public void shouldAttachSubscriptionIdentityHeadersWhenItIsEnabled() {
        ...
        // then
        subscriber.waitUntilRequestReceived(request -> {
            Assertions.assertThat(request.getHeader("Hermes-Topic-Name")).isEqualTo(topic.getQualifiedName());
            Assertions.assertThat(request.getHeader("Hermes-Subscription-Name")).isEqualTo("batchSubscription");
        });
    }
    ...
    private void publishAll(String topicQualifiedName, TestMessage... messages) {
        stream(messages).forEach(message -> hermes.api().publishUntilSuccess(topicQualifiedName, message.body()));
    }

    private void expectSingleBatch(TestSubscriber subscriber, TestMessage... expectedContents) {
        subscriber.waitUntilRequestReceived(message -> {
            ...
            for (int i = 0; i < expectedContents.length; i++) {
                Assertions.assertThat(batch.get(i).get("message")).isEqualTo(expectedContents[i].getContent());
                Assertions.assertThat((String) ((Map) batch.get(i).get("metadata")).get("id")).isNotEmpty();
            }
        });
    }
