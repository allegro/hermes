...
public class StatsTest {
    ...
    public void shouldGetStats() {
        ...
        // then
        assertThat(stats.getTopicStats().getTopicCount()).isEqualTo(initalStats.getTopicStats().getTopicCount() + 3);
        assertThat(stats.getTopicStats().getAckAllTopicCount()).isEqualTo(initalStats.getTopicStats().getAckAllTopicCount() + 2);
        assertThat(stats.getTopicStats().getTrackingEnabledTopicCount()).isEqualTo(initalStats.getTopicStats().getTrackingEnabledTopicCount() + 2);
        assertThat(stats.getTopicStats().getAvroTopicCount()).isEqualTo(initalStats.getTopicStats().getAvroTopicCount() + 1);

        assertThat(stats.getSubscriptionStats().getSubscriptionCount()).isEqualTo(initalStats.getSubscriptionStats().getSubscriptionCount() + 3);
        assertThat(stats.getSubscriptionStats().getTrackingEnabledSubscriptionCount()).isEqualTo(initalStats.getSubscriptionStats().getTrackingEnabledSubscriptionCount() + 1);
        assertThat(stats.getSubscriptionStats().getAvroSubscriptionCount()).isEqualTo(initalStats.getSubscriptionStats().getAvroSubscriptionCount() + 2);
    }
    ...
    private Subscription subscription(String topicQualifiedName, ContentType contentType, TrackingMode trackingMode) {
    ...
    private Stats getStats() {
        return hermes.api().getManagementStats().expectStatus().isOk().expectBody(Stats.class).returnResult().getResponseBody();
    }
