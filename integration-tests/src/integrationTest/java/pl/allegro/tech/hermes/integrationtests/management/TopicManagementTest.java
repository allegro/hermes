...
public class TopicManagementTest {
  ...
  public void
    ...
    hermes
        .api()
        .saveSchema(topic.getQualifiedName(), false, AvroUserSchemaLoader.load().toString());

    // when
    PatchData patch =
        patchData().set("contentType", ContentType.AVRO).set("migratedFromJsonType", true).build();
    WebTestClient.ResponseSpec response = hermes.api().updateTopic(topic.getQualifiedName(), patch);
  ...
  public void shouldReturnTopicsThatAreCurrentlyTracked() {
    // given
    Topic trackedTopic =
        hermes.initHelper().createTopic(topicWithRandomName().withTrackingEnabled(true).build());
  ...
  public void shouldReturnTopicsThatAreCurrentlyTrackedForGivenGroup() {
    ...
    Topic trackedTopic =
        hermes
            .initHelper()
            .createTopic(topic(groupName, "trackedTopic1").withTrackingEnabled(true).build());
    hermes.initHelper().createTopic(topicWithRandomName().withTrackingEnabled(false).build());
  ...
  public void shouldReturnTrackedTopicsWithAvroContentType() {
    ...
    Topic untrackedJsonTopic =
        hermes
            .initHelper()
            .createTopic(
                topicWithRandomName().withTrackingEnabled(false).withContentType(JSON).build());
    Topic trackedJsonTopic =
        hermes
            .initHelper()
            .createTopic(
                topicWithRandomName().withTrackingEnabled(true).withContentType(JSON).build());
        ...
        .forEach(
            topicName -> {
              ...
              WebTestClient.ResponseSpec response =
                  hermes
                      .api()
                      .createTopic(new TopicWithSchema(topic(group, topicName).build(), null));
  ...
  public void shouldCreateTopicWithRestrictedSubscribing() {
    // given
    TopicWithSchema topic =
        new TopicWithSchema(topicWithRandomName().withSubscribingRestricted().build(), null);
    hermes.initHelper().createGroup(Group.from(topic.getName().getGroupName()));
  ...
  public void shouldNotAllowNonAdminUserToDisableFallbackToRemoteDatacenter() {
    ...
    Topic topic =
        hermes
            .initHelper()
            .createTopic(topicWithRandomName().withFallbackToRemoteDatacenterEnabled().build());
    TestSecurityProvider.setUserIsAdmin(false);
    PatchData patchData =
            ...
            topicWithRandomName()
                .withPublishingChaosPolicy(
                    new PublishingChaosPolicy(
                        DATACENTER, null, Map.of("dc1", new ChaosPolicy(100, 100, 99, false))))
                .build());
    hermes.initHelper().createGroup(Group.from(topic.getName().getGroupName()));
        ...
        hermes
            ...
            .createTopic(
                topicWithRandomName()
                    .withPublishingChaosPolicy(
                        new PublishingChaosPolicy(
                            DATACENTER, null, Map.of("dc1", new ChaosPolicy(100, 100, 100, false))))
                    .build());
    PatchData patchData =
        PatchData.from(
            ImmutableMap.of(
                "chaos",
                ImmutableMap.of(
                    "datacenterPolicies", ImmutableMap.of("dc1", ImmutableMap.of("delayTo", 99)))));
  ...
  public static ErrorCode getErrorCode(WebTestClient.ResponseSpec createTopicResponse) {
    return Objects.requireNonNull(
            createTopicResponse.expectBody(ErrorDescription.class).returnResult().getResponseBody())
        .getCode();
  }
}
