...
public class MessageBufferLoadingTest {
  ...
  private static final HermesConsumersTestApp consumers =
      new HermesConsumersTestApp(infra.hermesZookeeper(), infra.kafka(), infra.schemaRegistry());
  ...
  private void backupFileWithOneMessage(String tempDirPath, Topic topic) {
    ...
    byte[] content =
        wrapper.wrapJson(
            "message".getBytes(defaultCharset()), messageId, timestamp, Collections.emptyMap());

    messageRepository.save(
        new JsonMessage(messageId, content, timestamp, null, Collections.emptyMap()), topic);
