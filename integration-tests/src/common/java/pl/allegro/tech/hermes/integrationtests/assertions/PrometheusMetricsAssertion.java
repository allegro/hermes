...
public class PrometheusMetricsAssertion {
    ...
    private static final Pattern METRIC_LINE_PATTERN = Pattern.compile("^[a-z_]+\\{(.*)\\} (\\d+\\.\\d+)$");
    ...
    public PrometheusMetricWithNameAssertion contains(String metricName) {
        List<String> matchedLines = actualBody.lines()
    ...
    public static class PrometheusMetricWithNameAssertion {
        ...
        private PrometheusMetricAssertion withLabels(String[] names, String[] values) {
            List<String> matchedLines = actualMetrics.stream()
                    .filter(line -> {
                    ...
                    .toList();
            assertThat(matchedLines).overridingErrorMessage("There is no metric with provided labels").isNotEmpty();
            assertThat(matchedLines).overridingErrorMessage("Found more than one metric with provided labels").hasSize(1);
