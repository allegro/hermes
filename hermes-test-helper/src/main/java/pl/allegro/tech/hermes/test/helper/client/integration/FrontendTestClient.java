...
public class FrontendTestClient {
    ...
    public FrontendTestClient(int frontendPort) {
        ...
        this.chunkedClient =
                newClient(new ClientConfig().property(REQUEST_ENTITY_PROCESSING, CHUNKED));
    }
    ...
    public Response publishChunked(String topicQualifiedName, String body) {
        return chunkedClient
                .target(
                        UriBuilder.fromUri(frontendContainerUrl)
                                .path(TOPIC_PATH)
    ...
    public int publishJSONUntilSuccess(
        ...
        waitAtMost(Duration.ofSeconds(10))
                .untilAsserted(
                        () -> {
                            attempts.getAndIncrement();
                            publishJSON(topicQualifiedName, body, headers)
                                    .expectStatus()
                                    .isCreated();
                        });
        return attempts.get();
    ...
    public int publishAvroUntilSuccess(
        ...
        waitAtMost(Duration.ofSeconds(10))
                .untilAsserted(
                        () -> {
                            attempts.getAndIncrement();
                            publishAvro(topicQualifiedName, body, headers)
                                    .expectStatus()
                                    .isCreated();
                        });
        return attempts.get();
