...
class ConsumerGroupsDescriber {
    ...
    Optional<ConsumerGroup> describeConsumerGroup(Topic topic, String subscriptionName) {
        ConsumerGroupId consumerGroupId =
                kafkaNamesMapper.toConsumerGroupId(
        ...
        Optional<ConsumerGroupDescription> description =
                adminClient
                        .describeConsumerGroups(
                                Collections.singletonList(consumerGroupId.asString()))
                        .all()
                        .get()
                        .values()
                        .stream()
                        ...
                        .map(
                                topicPartition -> {
                                    Optional<OffsetAndMetadata> offset =
                                            Optional.ofNullable(
                                                    topicPartitionOffsets.get(topicPartition));
                                    return new pl.allegro.tech.hermes.api.TopicPartition(
                                            ...
                                            kafkaTopicContentTypes.get(
                                                    KafkaTopicName.valueOf(
                                                            topicPartition.topic())));
                                })
                        .collect(toSet());
