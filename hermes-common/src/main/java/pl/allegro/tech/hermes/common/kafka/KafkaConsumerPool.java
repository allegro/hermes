...
/**
 * This class help us to avoid unnecessarily creating new kafka consumers for the same broker
 * instance, mainly in case of retransmission and migrating topic from json to avro. We map topic
 * and its partitions to specific kafka broker which without caching would lead to creating exactly
 * the same consumer multiple times in short period of time. Additionaly consumers created here are
 * rarely used, so we don't bother about concurrency and thread safety.
 */
public class KafkaConsumerPool {
    ...
    public KafkaConsumerPool(
            KafkaConsumerPoolConfig poolConfig,
    ...
    public KafkaConsumer<byte[], byte[]> get(String topicName, int partition) {
        try {
            int leaderId =
                    brokerStorage.readLeaderForPartition(new TopicPartition(topicName, partition));
            return kafkaConsumers.get(leaderId);

        } catch (ExecutionException e) {
            String message =
                    String.format(
                            "Cannot get KafkaConsumer for topic %s and partition %d",
                            topicName, partition);
    ...
    private static class KafkaConsumerSupplier
            extends CacheLoader<Integer, KafkaConsumer<byte[], byte[]>> {

        private final KafkaConsumerPoolConfig poolConfig;
        private final String configuredBootstrapServers;

        KafkaConsumerSupplier(
                KafkaConsumerPoolConfig poolConfig, String configuredBootstrapServers) {
        ...
        private KafkaConsumer<byte[], byte[]> createKafkaConsumer() {
            ...
            props.put(
                    GROUP_ID_CONFIG,
                    poolConfig.getIdPrefix() + "_" + poolConfig.getConsumerGroupName());
    ...
    private static class KafkaConsumerRemoveListener
            implements RemovalListener<Integer, KafkaConsumer<byte[], byte[]>> {
        @Override
        public void onRemoval(
                RemovalNotification<Integer, KafkaConsumer<byte[], byte[]>> notification) {
