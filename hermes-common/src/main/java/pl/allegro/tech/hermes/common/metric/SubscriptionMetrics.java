...
public class SubscriptionMetrics {
    ...
    public HermesTimer latency(SubscriptionName subscription) {
        return HermesTimer.from(
                meterRegistry.timer(
                        SubscriptionMetricsNames.SUBSCRIPTION_LATENCY,
    ...
    public <T> void registerInflightGauge(
        ...
        meterRegistry.gauge(
                SubscriptionMetricsNames.SUBSCRIPTION_INFLIGHT,
                subscriptionTags(subscription),
                obj,
                f);
    }
    ...
    public HermesTimer consumerIdleTimer(SubscriptionName subscription) {
        return HermesTimer.from(
                meterRegistry.timer(
                        SubscriptionMetricsNames.SUBSCRIPTION_IDLE_DURATION,
                        subscriptionTags(subscription)));
    }
    ...
    public HermesCounter filteredOutCounter(SubscriptionName subscription) {
        return HermesCounters.from(
                micrometerCounter(
                        SubscriptionMetricsNames.SUBSCRIPTION_FILTERED_OUT, subscription));
    }
    ...
    public HermesCounter otherErrorsCounter(SubscriptionName subscription) {
        return HermesCounters.from(
                micrometerCounter(
                        SubscriptionMetricsNames.SUBSCRIPTION_OTHER_ERRORS, subscription));
    }
    ...
    public static class SubscriptionMetricsNames {
        ...
        public static final String SUBSCRIPTION_HTTP_STATUS_CODES =
        ...
        public static final String SUBSCRIPTION_INFLIGHT_TIME =
                "subscription.inflight-time-seconds";
    }
}
