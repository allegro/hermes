...
public class JsonMessageContentWrapper implements MessageContentWrapper {
    ...
    @Override
    public MessageWithMetadata unwrapContent(byte[] json) {
        int rootIndex = indexOf(json, contentRootField);
        int metadataIndex = indexOf(json, metadataRootField);
        try {
            return new MessageWithMetadata(unwrapMesssageMetadata(json, metadataIndex, rootIndex), unwrapContent(json, rootIndex));
