...
public class ZookeeperUndeliveredMessageLogTest extends ZookeeperBaseTest {
    ...
    private void assertThatMetricsHaveBeenReported(int persistedMessageCount) {
        ...
        assertThat(metricValue(meterRegistry, PERSISTED_UNDELIVERED_MESSAGES_METER, Search::counter, Counter::count)
                .orElse(0.0d)).isEqualTo(persistedMessageCount);
        assertThat(metricValue(meterRegistry, PERSISTED_UNDELIVERED_MESSAGE_SIZE + ".bytes",
                Search::summary, DistributionSummary::count).orElse(0L)).isEqualTo(persistedMessageCount);
    }
}
