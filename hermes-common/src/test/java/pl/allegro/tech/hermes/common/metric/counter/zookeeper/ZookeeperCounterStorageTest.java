...
public class ZookeeperCounterStorageTest {
    ...
    @Mock private SharedCounter sharedCounter;
    ...
    public void initialize() {
        ...
        storage =
                new ZookeeperCounterStorage(
                        sharedCounter, subscriptionRepository, pathCompiler, "/hermes");
    }
    ...
    public void shouldReadValueFromTopicMetric() {
        // given
        when(sharedCounter.getValue("/hermes/groups/test/topics/topic/metrics/published"))
                .thenReturn(10L);
    ...
    public void shouldIncrementSubscriptionMetricUsingSharedCounter() {
        // given when
        storage.setSubscriptionDeliveredCounter(
                TopicName.fromQualifiedName("test.topic"), "sub", 10);

        // then
        verify(sharedCounter)
                .increment(
                        "/hermes/groups/test/topics/topic/subscriptions/sub/metrics/delivered", 10);
    }

    @Test
    public void shouldReadValueFromSubscriptionMetric() {
        // given
        when(sharedCounter.getValue(
                        "/hermes/groups/test/topics/topic/subscriptions/sub/metrics/delivered"))
                .thenReturn(10L);

        // when
        long value =
                storage.getSubscriptionDeliveredCounter(
                        TopicName.fromQualifiedName("test.topic"), "sub");
