...
public class AvroMessageContentWrapperTest {
    ...
    public void shouldWrapAndUnwrapAvroMessageWithMetadata() {
        ...
        byte[] wrappedMessage =
                avroMessageContentWrapper.wrapContent(
                        content, id, timestamp, avroUser.getSchema(), Collections.emptyMap());
        UnwrappedMessageContent unwrappedMessageContent =
                avroMessageContentWrapper.unwrapContent(
                        wrappedMessage, avroUser.getCompiledSchema());

        // then
        assertThat(unwrappedMessageContent.getMessageMetadata().getId()).isEqualTo(id);
        assertThat(unwrappedMessageContent.getMessageMetadata().getTimestamp())
                .isEqualTo(timestamp);
    ...
    public void shouldWrappedMessageBeUnchangedWhenSchemaNotContainsMetadata() throws IOException {
        // when
        Schema schemaWithoutMetadata =
                new Schema.Parser()
                        .parse(
                                IOUtils.toString(
                                        getClass()
                                                .getResourceAsStream(
                                                        "/schema/user_no_metadata.avsc"),
                                        StandardCharsets.UTF_8));
        byte[] wrappedMessage =
                avroMessageContentWrapper.wrapContent(
                        content, id, timestamp, schemaWithoutMetadata, Collections.emptyMap());
    ...
    public void shouldUnwrapAvroMessageAndGenerateMetadataWhenNotExists() throws Throwable {
        ...
        // when
        UnwrappedMessageContent unwrappedMessage =
                avroMessageContentWrapper.unwrapContent(
                        wrappedMessage, avroUser.getCompiledSchema());
    ...
    @Test
    public void shouldUnwrapAvroMessageAndSetEmptyMessageIdWhenNotGivenInMetadata()
            throws Throwable {
        // given
        byte[] wrappedMessage =
                wrapContentWithoutMessageIdInMetadata(content, avroUser.getSchema());
    ...
    private byte[] wrapContentWithoutMessageIdInMetadata(byte[] message, Schema schema)
            throws Exception {
        return wrapContent(message, schema, metadataMapWithoutMessageId(timestamp));
    }

    private byte[] wrapContent(byte[] message, Schema schema, Map<Utf8, Utf8> metadata)
            throws Exception {
