...
public class InterpolatingEndpointAddressResolverTest {
    ...
    private final InterpolatingEndpointAddressResolver resolver =
            new InterpolatingEndpointAddressResolver(interpolator);

    private final EndpointAddressResolverMetadata metadata =
            EndpointAddressResolverMetadata.empty();

    @Test
    public void shouldUseInterpolatorToInterpolateURI()
            throws InterpolationException, EndpointAddressResolutionException {
        // given
        ...
        when(interpolator.interpolate(address, message))
                .thenReturn(URI.create("http://localhost/hello"));
    ...
    public void shouldThrowResolvingExceptionWhenInterpolationFails()
        ...
        // then
        assertThrows(
                EndpointAddressResolutionException.class,
                () ->
                        resolver.resolve(
                                EndpointAddress.of("http://localhost/{a}"),
                                withTestMessage()
                                        .withContent("content", StandardCharsets.UTF_8)
                                        .build(),
                                metadata));
    }
}
