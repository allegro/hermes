...
public class MessageSenderFactoryTest {
    ...
    private final ResilientMessageSender resilientMessageSender =
            Mockito.mock(ResilientMessageSender.class);
    ...
    public void shouldCreateCustomProtocolMessageSender() {
        ...
        MessageSenderFactory factory =
                new MessageSenderFactory(
                        ImmutableList.of(
                                protocolMessageSenderProviderReturning(
                                        referenceMessageSender, "myProtocol")));
    ...
    public void shouldGetProtocolNotSupportedExceptionWhenPassingUnknownUri() {
        ...
        Subscription subscription =
                subscription("group.topic", "subscription", "unknown://localhost:8080/test")
                        .build();

        // then
        assertThrows(
                EndpointProtocolNotSupportedException.class,
                () -> factory.create(subscription, resilientMessageSender));
    }

    private ProtocolMessageSenderProvider protocolMessageSenderProviderReturning(
            Object createdMessageSender, String protocol) {
        return new ProtocolMessageSenderProvider() {
            @Override
            public MessageSender create(
                    Subscription endpoint, ResilientMessageSender resilientMessageSender) {
