...
public class OffsetCommitter {
    ...
    public Set<SubscriptionPartitionOffset> calculateOffsetsToBeCommitted(
        ...
        try (HermesTimerContext ignored = timer.time()) {
            ...
            Map<SubscriptionPartition, Long> maxDrainedProcessedOffsets =
                    processedOffsetsReducer.reduced;
            ...
            for (SubscriptionPartition partition :
                ...
                if (partitionAssignmentState.isAssignedPartitionAtCurrentTerm(partition)) {
                    ...
                    if (offsetToBeCommitted >= 0 && offsetToBeCommitted < Long.MAX_VALUE) {
                        ...
                        offsetsToCommit.add(
                                new SubscriptionPartitionOffset(partition, offsetToBeCommitted));

                        // if we just committed the maximum possible offset for partition, we can
    ...
    private void drain(
        ...
        for (int i = 0; i < size; i++) {
            ...
            if (element != null) {
                ...
                logger.warn(
                        "Unexpected null value while draining queue [idx={}, size={}]", i, size);
                break;
            }
        }
    }
