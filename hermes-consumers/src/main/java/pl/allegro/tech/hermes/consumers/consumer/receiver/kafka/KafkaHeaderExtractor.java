    ...
    public Map<String, String> extractExternalMetadata(
        ...
        if (isHTTPheadersPropagationAsKafkaHeadersEnabled) {
            ...
            if (httpHeaders.isEmpty()) {
                // After completing the migration to the approach with Kafka headers, we should
                // remove this condition.
                return defaultExternalMetadata;
            }
            // The following is necessary to be compatible with building external metadata based on
            // the message body.
            // See:
            // pl.allegro.tech.hermes.common.message.wrapper.AvroMessageContentWrapper.getMetadata
            Map<String, String> externalMetadata = new HashMap<>(httpHeaders);
            externalMetadata.put(
                    AvroMetadataMarker.METADATA_MESSAGE_ID_KEY.toString(),
