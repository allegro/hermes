...
public class JettyBroadCastMessageSender implements MessageSender {
    ...
    private Set<CompletableFuture<SingleMessageSendingResult>> collectResults(Message message)
        ...
        // add previously succeeded uris to the result set so that successful uris from all attempts
        // are retained.
        // this way a MessageSendingResult can be considered successful even when the last send
        // attempt
        ...
        for (String succeededUri : message.getSucceededUris()) {
            try {
                ...
                logger.error(
                        "Error while parsing already sent broadcast URI {}",
                        succeededUri,
                        exception);
            }
        }
    ...
    private CompletableFuture<SingleMessageSendingResult> processResponse(Request request) {
        return resilientMessageSender.send(
                resultFuture ->
                        request.send(
                                sendingResultHandlers.handleSendingResultForBroadcast(
                                        resultFuture)),
                exceptionMapper);
    }
