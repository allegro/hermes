...
public class ByteBufferMessageBatchFactory implements MessageBatchFactory {
    ...
    public ByteBufferMessageBatchFactory(
        ...
        metrics.consumer()
                .registerBatchBufferAvailableBytesGauge(
                        bufferPool, DirectBufferPool::availableMemory);
    }
    ...
    public MessageBatch createBatch(Subscription subscription) {
        try {
            ...
            switch (subscription.getContentType()) {
                case JSON:
                    return new JsonMessageBatch(
                            randomUUID().toString(), buffer, subscription, clock);
                case AVRO:
                default:
                    throw new UnsupportedOperationException(
                            "Batching is not supported yet for contentType "
