...
export function useConsumerGroups(topicName: string, subscriptionName: string): UseConsumerGroups {
  ...
  const fetchConsumerGroups = async () => {
    try {
      loading.value = true;
      consumerGroups.value = (await getConsumerGroups(topicName, subscriptionName)).data;
    } catch (e) {
