const en_US = {
  topicView: {
    ...
    properties: {
      ...
      tooltips: {
        acknowledgement: "Specifies the strength of guarantees that acknowledged message was indeed persisted. In " +
        ...
        retentionTime: "For how many hours/days message is available for subscribers after being published.",
        authorizedPublishers: "When authorisation is enabled, only authenticated services are allowed to publish on this topic.",
        allowUnauthenticatedAccess: "Allowing unauthenticated access should be enabled only when migrating topic to " +
          "authorised, so both authenticated and unauthenticated clients are allowed to publish simultaneously.",
        restrictSubscribing: "When subscribing is restricted, only owner of this topic can create new subscriptions.",
        storeOffline: "Should data from this topic be stored in offline storage (e.g. HDFS).",
        offlineRetention: "For how long should this topic be stored in offline storage."
      }
    }
  }
};
